...
import { ArrowRight, Check, CircleDollarSign, ListTodo, Waves } from "lucide-react";
        ...
        <section className="mb-6 grid gap-4 md:grid-cols-[1.3fr_0.7fr]">
          <div className="rounded-[2rem] border border-white/10 bg-[rgba(9,17,25,0.76)] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.35)]">
            ...
            <div className="text-3xl font-semibold text-white">
              {currentTrack?.name ?? "还没有主线"}
            </div>
            <div className="mt-3 text-lg text-[var(--accent-3)]">
              {currentPhase?.name ?? "先去详细页添加第一阶段"}
            </div>
            <div className="mt-5 h-2 overflow-hidden rounded-full bg-white/8">
              <div
                className="h-full rounded-full bg-[linear-gradient(90deg,var(--accent-2),var(--accent-3))]"
                style={{ width: `${currentTrack?.overallPercent ?? 0}%` }}
              />
            </div>
            <div className="mt-2 text-sm text-slate-400">
              进度 {currentTrack?.overallPercent ?? 0}%
            </div>
          </div>

          <div className="rounded-[2rem] border border-white/10 bg-[rgba(9,17,25,0.76)] p-6 shadow-[0_20px_70px_rgba(0,0,0,0.35)]">
            <div className="mb-3 flex items-center gap-2 text-sm text-slate-300">
              <CircleDollarSign className="h-4 w-4 text-[var(--accent-3)]" />
              恒力
            </div>
              ...
              tasks.map((task) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="inline-flex shrink-0 items-center justify-center gap-2 rounded-full bg-[linear-gradient(135deg,var(--accent-2),var(--accent-3))] px-4 py-2 text-sm font-semibold text-slate-950 transition hover:brightness-110"
                  >
